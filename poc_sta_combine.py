    ...
    for company_file in company_file_names_sta:
        ...
        for filename in filenames:
            company_df1 = pd.read_csv(
                il.cwd + r"\result\\" + company_file + "_result_combine_" + filename + symbolstr + poctype + "_till" + lastdate_str + ".csv")
...
def company_detail_concat(formaldate_str, company_file_names_list, poctype, lastdate_str, symbolstr, symbolstr2=""):
    '''
        连接所有用户的每日市值表格
    '''
    ...
    for company_file in company_file_names_list:
        try:
            company_df1 = pd.read_csv(
                il.cwd + r"\result\\" + company_file + "_result_combine_" + symbolstr2 + poctype + "_till" + formaldate_str + ".csv")
            ...
            company_nofee_df1 = pd.read_csv(
                il.cwd + r"\result\\" + company_file + "_result_combine_nofee_" + symbolstr2 + poctype + "_till" + formaldate_str + ".csv")
            ...
            company_net_df1 = pd.read_csv(
                il.cwd + r"\result\\" + company_file + "_result_combine_" + symbolstr2 + poctype + "_till" + formaldate_str + ".csv")
            company_net_df2 = pd.read_csv(
                il.cwd + r"\result\\" + company_file + "_result_combine_" + symbolstr + poctype + "_" + lastdate_str + ".csv")
            company_net_df1 = company_net_df1.ix[:, 1:]
            company_net_df2 = company_net_df2.ix[:, 1:]
            company_net_df = pd.concat([company_net_df1, company_net_df2], axis=1)
        ...
        print("File saved:",
              il.cwd + r"\result\\" + company_file + "_result_combine_nofee_" + symbolstr + poctype + "_till" + lastdate_str + ".csv")
        company_net_df.to_csv(
            il.cwd + r"\result\\" + company_file + "_result_combine_net_" + symbolstr + poctype + "_till" + lastdate_str + ".csv")
        print("File saved:",
              il.cwd + r"\result\\" + company_file + "_result_combine_net_" + symbolstr + poctype + "_till" + lastdate_str + ".csv")
...
def company_net_detail_concat(formaldate_str, company_file_names_list, poctype, lastdate_str, symbolstr, symbolstr2=""):
    '''
        连接所有用户的每日市值表格
    '''
    for company_file in company_file_names_list:
        company_net_df1 = pd.read_csv(
            il.cwd + r"\result\\" + company_file + "_result_combine_net_" + symbolstr2 + poctype  + ".csv")
        company_net_df2 = pd.read_csv(
...
def poc_detail_compute_combine(company_file_names_poc, poctype, users_inside_dic, datelist_in_poc_compute, symbolstr):
    ...
    for company_file in company_file_names_poc:
        ...
        users_test = pd.concat(
            [users_inside[106:107], users_inside[86:87]], axis=0)
        for index, row in users_inside.iterrows():
            ...
            for date in datelist_in_poc_compute:
                ...
                user_combination_date_dic, user_combination_date, buy_date = rl.getUserCombinationByDate(date,
                                                                                                         user_combination)
                # 如果当前用户的id大于100（说明是新增调仓用户），如果其没有配置信息，则用与其风险等级相同的有配置信息的用户配置替代
                if user_combination_date.empty and int(userid) > 100:
                ...
                if user_combination_date.empty:
                    print("User " + userid + " and users with same risk have no combination on " + date + " or before.")
                else:
                    if not bool(user_funds_hold):
                        # 如果用户持仓情况为空仓，则买入基金
...
if __name__ == '__main__':
    ...
    for poctype_out in poctype_out_list:
        ...
        company_file_names_poc = ["betago","xj","kmrd","sz"]
        ...
        date_pairs = [("2017-07-01", "2017-10-29"), ("2017-10-30", "2017-11-22"), ("2017-11-27", "2017-12-31")]
        startdate_poc = "2017-07-01"
        enddate_poc = "2017-11-04"
        ...
        # company_net_detail_concat(startdate_poc, company_file_names_poc, poctype_out, enddate_poc, symbolstr)
