...
def poc_detail_compute_combine(company_file_names_poc, poctype, users_inside):
    ...
    for company_file in company_file_names_poc:
                ...
                if not bool(user_funds_hold):
                    if date not in user_combination["date"].values.tolist():
                        ...
                        if date.replace("-", "") not in datelist_possible:
                            ...
                            user_marketcap_nofee[date] = usermoney
                    ...
                    if date not in user_combination["date"].values.tolist():
                        if date.replace("-", "") not in datelist_possible:
                            ...
                            user_marketcap_nofee[date] = user_marketcap_value + leftusermoney_nofee
                            ...
                            if holdeticker in funds_net["ticker"].values.tolist():
                                ...
                                if hold_fund_net == 0:
                                    hold_fund_net = rl.getFundsNetBefore_byTickerDate(holdeticker,
                                                                                      date.replace("-", ""),
                                                                                      funds_net, "%Y%m%d")
                                fund_marketcap_nofee = fund_marketcap = float(holdamount) * float(hold_fund_net)
                                ...
                                if hold_fund_net == 0:
                                    hold_fund_net = rl.getFundsNetBefore_byTickerDate(holdeticker,
                                                                                      date.replace("-", ""),
                                                                                      funds_net, "%Y%m%d")
                                fund_marketcap_nofee = fund_marketcap = holdamount + (
                                                                                         holdamount / 10000) * hold_fund_net
                            else:
                                funds_no_netdata.append(fund_ticker)
                            user_marketcap_value = user_marketcap_value + fund_marketcap
        ...
        print("File saved:", il.cwd + r"\result\\" + company_file + "_result_nofee_" + poctype + ".csv")
