...
def poc_sta(datepairs, poctype, company_file_names_sta):
    '''
    ...
    for company_file in company_file_names_sta:
        ...
        for startday_str_sta, endday_str_sta in datepairs:
            ...
            company_df1 = pd.read_csv(il.cwd + r"\result\\" + company_file + "_result_" + poctype + ".csv")
            company_net_df1 = pd.read_csv(il.cwd + r"\result\\" + company_file + "_result_net_" + poctype + ".csv")
...
def poc_maxdown(company_file_names_maxdown, poctype):
    '''
        计算不同厂家给出的配置计算最大回撤，并输出为文件,不计算费率和赎回过程中的资金延迟
    '''
    company_detial = pd.DataFrame()
    for company_file in company_file_names_maxdown:
        company_total_detial_df = pd.read_csv(il.cwd + r"\result\\" + company_file + "_result_" + poctype + ".csv",
                                              dtype=float64)
        company_total_detial_df = company_total_detial_df.ix[:, 1:]
        maxdown_user_dic = {}
        iloc_index = 0
        time_cost = 0
        for index, row in company_total_detial_df.T.iterrows():
            if (iloc_index + 1) < len(company_total_detial_df.T):
                iloc_index += 1
                start = time.clock()
                left_df = company_total_detial_df.T.iloc[iloc_index:]
                left_df_describe = left_df.describe()
                left_se_max = left_df_describe.T["min"]
                down_se = (row - left_se_max) / row
                if not bool(maxdown_user_dic):
                ...
                if iloc_index % 10 == 0:
                    ...
                    print("Time Left Estimated:",
                          (time_cost / (int(iloc_index))) * len(company_total_detial_df.T) - time_cost)
        maxdown_user_dic_positive = {w: maxdown_user_dic[w] if maxdown_user_dic[w] > 0 else 0 for w
                                     in maxdown_user_dic.keys()}
        company_detial = company_detial.append(maxdown_user_dic_positive, ignore_index=True)
    company_detial = company_detial.T
    company_detial.columns = company_file_names_maxdown
    print(company_detial)
    company_detial.to_csv(il.cwd + r"\result\\all_company_result_maxdown_" + poctype + ".csv")
    print("File saved:", il.cwd + r"\result\\all_company_result_maxdown_" + poctype + ".csv")
...
def poc_detail_compute_combine(company_file_names_poc, poctype, users_inside, symbolstr,datelist_inside):
    ...
    for company_file in company_file_names_poc:
        ...
        company_detial.to_csv(
            il.cwd + r"\result\\" + company_file + "_result_combine_" + symbolstr + "_" + poctype + "_till" + datelist_inside[
                -1] + ".csv")
        print("File saved:",
              il.cwd + r"\result\\" + company_file + "_result_combine_" + symbolstr + "_" + poctype + "_till" +
              datelist_inside[-1] + ".csv")
        company_detial_nofee.to_csv(
            il.cwd + r"\result\\" + company_file + "_result_combine_nofee_" + symbolstr + "_" + poctype + "_till" +
            datelist_inside[-1] + ".csv")
        print("File saved:",
              il.cwd + r"\result\\" + company_file + "_result_combine_nofee_" + symbolstr + "_" + poctype + "_till" +
              datelist_inside[-1] + ".csv")
        company_detial_net.to_csv(
            il.cwd + r"\result\\" + company_file + "_result_combine_net_" + symbolstr + "_" + poctype + "_till" +
            datelist_inside[-1] + ".csv")
        print("File saved:",
              il.cwd + r"\result\\" + company_file + "_result_combine_net_" + symbolstr + "_" + poctype + "_till" +
              datelist_inside[-1] + ".csv")
...
if __name__ == '__main__':
    ...
    for poctype_out in poctype_out_list:
        ...
        date_pairs = [("2017-07-01", "2017-10-30")]
